...
<div class="theorem">
    ...
    \[
        oneTimePadDecrypt \; n \; (oneTimePadDecrypt \; n \; M \; K) \; K = M
    \]
</div>


Using dependent function types in Idris we can *encode this theorem as a type*. To see how, we have to understand how to encode the equality in the above theorem. The equality above is very different from the equality when we define functions. For instance, in the function definition `xor ZeroBit ZeroBit = ZeroBit` means that `xor ZeroBit ZeroBit`*by definition* is the same as `ZeroBit`. However, the equality in the above theorem might or might not actually be true, thus we regard it as a **proposition**, a potential fact yet to be proven.

In Idris we represent a proposition as a type, and we prove that the proposition is true by showing that there exists something of that type. In this case, 


In Idris this is represented as a type, which we write in Idris as `oneTimePadDecrypt n (oneTimePadDecrypt n M K) K = M`. Note that this type depends on `n`, `M` and `K` so `oneTimePadDecrypt (S n) (oneTimePadDecrypt (S n) M K) K = M` would be a different type. We say that one of these propositions (types) is true if there exists something of that type, where that something is called a **witness** to the proposition. 
